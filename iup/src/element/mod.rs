...
macro_rules! impl_element_nofrom {
    ($ty_path:path, $classname:expr) => {
        ...
        unsafe impl Sync for $ty_path {}

        unsafe impl Send for $ty_path {}

        impl $crate::callback::DestroyCb for $ty_path {}

        impl $crate::callback::PostMessageCb for $ty_path {}
    };
}

pub struct User(*mut iup_sys::Ihandle);

impl User {
    pub fn new() -> Self {
        unsafe { User::from_raw(iup_sys::IupUser()) }
    }
}

impl_widget_container!(User, "user");
...
pub trait Element : Sized + Copy + Clone {
    ...
    fn from_raw(ih: *mut iup_sys::Ihandle) -> Self {
        if ih.is_null() {
            ...
            unsafe {
                ...
                iup_sys::IupSetCallback(ih, cstr!("DESTROY_CB"), Some(on_element_destroy));
    ...
    fn set_attrib<S1, S2>(&self, name: S1, value: S2) -> Self
    ...
    fn set_attrib_id<S1, S2>(&self, name: S1, value: S2) -> Self
                                        where S1: Into<String>, S2: Into<String> {
        // The way IupSetAttribute works is infeasible to safety. Use IupSetStrAttribute.
        let cname = CString::new(name.into()).unwrap();
        let cvalue = CString::new(value.into()).unwrap();
        unsafe { iup_sys::IupSetAttribute(self.raw(), cname.as_ptr(), cvalue.as_ptr()) };
        self.clone()
    }
    ...
    fn attrib_id<S: Into<String>>(&self, name: S, id: i32) -> Option<String> {
        // Notice IupGetAttribute does not really give strings but pointers (that may be anything)
        // most (if not all) the default IUP attributes are string values, so we are safe by
        // defaulting to IupGetAttribute. A method should be defined to deal with raw attributes.
        let cname = CString::new(name.into()).unwrap();
        match unsafe { iup_sys::IupGetAttributeId(self.raw(), cname.as_ptr(), id) } {
            cvalue if cvalue.is_null() => None,
            cvalue => Some(string_from_cstr!(cvalue)),
        }
    }
    ...
    /// statically).
    fn set_attrib_data<S1>(&self, name: S1, data: *const c_void) -> Self
    ...
    fn attrib_data<S1>(&self, name: S1) -> *mut c_void
                                       where S1: Into<String> {
        let cname = CString::new(name.into()).unwrap();
    ...
    /// with the attribute.
     fn set_attrib_handle<S1, E>(&self, name: S1, elem: E) -> Self
    ...
    fn attrib_handle<S1>(&self, name: S1) -> Option<Handle>
                                    where S1: Into<String> {
        let cname = CString::new(name.into()).unwrap();
    ...
    fn clear_attrib<S: Into<String>>(&self, name: S) -> Self {
        let cname = CString::new(name.into()).unwrap();
    ...
    fn reset_attrib<S: Into<String>>(&self, name: S) -> Self {
        let cname = CString::new(name.into()).unwrap();
